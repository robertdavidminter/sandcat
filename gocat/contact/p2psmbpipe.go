...
func (p2pPipeClient SmbPipeAPI) GetInstructions(profile map[string]interface{}) map[string]interface{} {
    output.VerbosePrint("[*] Going to fetch instructions.")
	...
	if err == nil {
		...
		for serverResp.MessageType == RESEND_REQUEST {
            ...
            if err != nil {
                output.VerbosePrint(fmt.Sprintf("[-] P2p resent beacon via %s: DEAD", profile["server"].(string)))
                break
            }
        }
        ...
        if msgIsEmpty(serverResp) {
            ...
            if out != nil {
                ...
                output.VerbosePrint(fmt.Sprintf("[*] P2p beacon via %s: ALIVE", profile["server"].(string)))
            } else {
